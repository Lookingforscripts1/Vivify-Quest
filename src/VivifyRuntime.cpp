...
private:
  ...
  void HandleCustomEvent(GlobalNamespace::BeatmapCallbacksController* callbackController,
    ...
    if (json == nullptr) {
      PaperLogger.warn("Vivify event [{}] did not contain custom data", std::string(type).c_str());
      return;
    }
  ...
  void WarnUnsupported(std::string_view type) {
    ...
    PaperLogger.warn("Vivify Quest does not support [{}] yet; skipping those events", key.c_str());
  }
  ...
  bool EnsureBeatmapPrepared(GlobalNamespace::BeatmapCallbacksController* callbackController) {
    ...
    if (customBeatmapData == nullptr) {
      PaperLogger.warn("Vivify received a custom event for a non-custom beatmap payload");
      return false;
    }
  ...
  void PrepareBeatmap(CustomJSONData::CustomBeatmapData* beatmapData) {
    ...
    if (beatmapData->customData != nullptr) {
      try {
        ...
        PaperLogger.warn("Vivify could not initialize Tracks associated data: {}", ex.what());
      }
    }
  ...
  void LoadMainBundle() {
    if (_selectedLevelPath.empty()) {
      if (_selectedMapHasVivifyRequirement) {
        PaperLogger.warn("Vivify level path was unavailable, so the Android asset bundle could not be loaded");
      }
      return;
    }
    ...
    if (_mainBundle == nullptr) {
      if (_selectedMapHasVivifyRequirement) {
        PaperLogger.warn("Vivify could not load [{}]", bundlePath.c_str());
      }
      return;
    }
    ...
    for (auto* customEventData : _currentBeatmapData->customEventDatas) {
      ...
      if (auto id = ReadStringView(*json, "id"); id.has_value()) {
        ...
        if (!seenIds.emplace(*data.id).second) {
          PaperLogger.warn("Vivify found duplicate InstantiatePrefab id [{}] in one beatmap", data.id->c_str());
        }
      }
  ...
  void InstantiatePrefab(CustomJSONData::CustomEventData* customEventData, rapidjson::Value const&) {
    ...
    if (data.instance == nullptr) {
      ...
      if (prefab == nullptr) {
        PaperLogger.warn("Vivify could not find prefab asset [{}]", data.asset.c_str());
        return;
      }
  ...
  void DestroyObjects(rapidjson::Value const& json) {
    for (auto const& id : ReadStringListOrSingle(json, "id")) {
      if (!DestroyPrefabById(id)) {
        PaperLogger.warn("Vivify could not destroy object [{}] because it is not currently active", id.c_str());
      }
    }
  }
  ...
  void HandleSetMaterialProperty(CustomJSONData::CustomEventData* customEventData, rapidjson::Value const& json) {
    ...
    if (material == nullptr) {
      PaperLogger.warn("Vivify could not find material asset [{}]", std::string(*asset).c_str());
      return;
    }
  ...
  void HandleSetAnimatorProperty(CustomJSONData::CustomEventData* customEventData, rapidjson::Value const& json) {
    ...
    if (prefabIt == _livePrefabs.end()) {
      PaperLogger.warn("Vivify could not find live prefab [{}] for animator properties", std::string(*id).c_str());
      return;
    }
