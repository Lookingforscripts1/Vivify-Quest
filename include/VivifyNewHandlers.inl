...
void HandleBlit(CustomJSONData::CustomEventData* customEventData, rapidjson::Value const& json) {
  ...
  if (asset.has_value()) {
    ...
    if (material == nullptr) {
      PaperLogger.warn("Vivify Blit could not find material [{}]", std::string(*asset).c_str());
      return;
    }
...
void HandleCreateScreenTexture(rapidjson::Value const& json) {
  ...
  PaperLogger.info("Vivify created screen texture [{}]", name.c_str());
}
...
if (cam.depthTexturePropertyId.has_value()) {
  ...
  PaperLogger.info("Vivify created camera [{}]", name.c_str());
}
...
void ParseAndApplyRenderSetting(std::string const& key, rapidjson::Value const& val,
  ...
  if (key == "fogColor" || key == "ambientLight" || key == "ambientSkyColor" ||
    ...
    if (val.IsArray() && val.Size() >= 3) {
      ...
      if (!immediate) {
        auto pd = MakePointDefinition(val.GetObject(), key, Tracks::ffi::WrapBaseValueType::Vec4);
        if (pd.has_value()) { RenderSettingValue rsv; rsv.name = key; rsv.kind = RenderSettingKind::Color; out.push_back(std::move(rsv)); }
      }
    }
    return;
  }
...
void HandleAssignObjectPrefab(CustomJSONData::CustomEventData*, rapidjson::Value const& json) {
  ...
  for (auto const& objType : objectTypes) {
    ...
    if (prefab == nullptr && !assetStr.empty()) {
      PaperLogger.warn("Vivify AssignObjectPrefab could not find asset [{}]", assetStr.c_str());
      continue;
    }
    ...
    PaperLogger.info("Vivify assigned object prefab [{}] for [{}]", assetStr.c_str(),
